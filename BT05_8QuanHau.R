...
place_queen <- function(queens=c()) { 
  ...
  # N???u có 8 quân h???u du???c d???t, thì dây ph???i là m???t gi???i pháp.  if (length(queens) == 8) {
    return(list(queens))
  }
...
plot_single_8queens <- function(queens, title = NULL) {
  ...
  p <- ggplot(queens_df, aes(rows, cols)) + 
    g?om_tile(data=board_df, aes(fill=check), colour='black') +
